<article class="flex gap-2">
  <section class="w-full max-w-150 p-6">
    <div class="flex flex-col mb-2 w-full">
      <h2 class="mt-4 text-xl font-bold text-[#16253b]">Bem-vindo de volta</h2>
      <p class="text-sm text-gray-600 mt-2">
        Faça login com seus dados para acessar sua conta.
      </p>
      <div class="flex flex-col sm:flex-row gap-3 mt-4 w-full">
        <button class="bg-white border border-gray-200 w-full flex items-center justify-center gap-2 px-4 py-4  cursor-pointer hover:opacity-85 rounded-sm hover:bg-gray-100 transition font-medium mt-4 ">
          ...
          <p class=" text-sm whitespace-nowrap">Entrar com Google</p>
        </button>
    ...
    <form id="form-login" class="modal-content w-full text-sm" method="POST" action="{{route('login')}}">
      ...
      <div class="flex flex-col gap-5 mb-2">
        <div class="w-full">
          <input name="email" type="email" class="px-3 py-4 border border-gray-200 rounded focus:ring-1 ring-[#16253b] outline-none w-full transition-all duration-250 ease-in" placeholder="E-mail" required>
        </div>
        <div class="w-full">
          <input name="password" type="password" class="px-3 py-4 border border-gray-200 rounded focus:ring-1 ring-[#16253b] outline-none w-full transition-all duration-250 ease-in" placeholder="Senha" required>
          <p class="error text-red-500 text-sm mt-1"></p>
        </div>
      </div>
      <div class="flex justify-between items-center mb-4">
        <label class="flex items-center text-sm text-[#16253b]">
          <input type="checkbox" name="remember" class="mr-2"> Lembrar-me
        </label>
        <a id="show-forgot-password" class="text-[#16253b] text-sm hover:underline cursor-pointer">Esqueceu a senha?</a>
      </div>
      <div class="flex w-full">
        <button type="submit" class="bg-[var(--color-primary)] transition-colors text-white cursor-pointer hover:opacity-85 px-6 py-4  rounded-sm font-semibold w-full">Entrar</button>
      </div>
      <p class="mt-4 text-center text-sm">
        Não tem conta?
        <button id="buttonRegister" type="button" class="text-primary  cursor-pointer text-[#16253b]">Registrar</button>
      </p>
    </form>
  </section>
