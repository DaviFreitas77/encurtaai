<form  id="form_shortened_url" class=" w-full max-w-[700px] text-sm">
    ...
    <div id="segmented-control" class="flex w-full max-w-xs p-1 mx-auto space-x-1 bg-gray-200 rounded-full">
        ...
        <button type="button" class="w-full px-4 py-2 text-sm font-semibold text-gray-900 bg-white rounded-full shadow-md focus:outline-none" aria-selected="true">
            Encurtar link
        </button>

        @auth
        <button type="button" class="w-full px-4 py-2 text-sm font-semibold text-gray-500 rounded-full hover:bg-white/60 focus:outline-none" aria-selected="false">
            Gerar QR Code
        </button>
        @else
        <button
